...
# UniF + Att
class UniF_Att(nn.Module):
	def __init__(self, emodict, worddict, embedding, args):
		super(UniF_Att, self).__init__()
	...
	def forward(self, sents, lengths):
		...
		if sents.size()[0] > 1:
			...
			for hop in range(self.hops):
				...
				attn_out, attn_weight = self.AttGRU[0](eps_mem, mem_bank, attn_hid, attn_mask)            # fix weight, hop=0
				attn_weights.append(attn_weight.squeeze(1))
				eps_mem = eps_mem + attn_out
				eps_mem = self.dropout_mid(eps_mem)
			s_out.append(eps_mem)
...
# BiF + Uni-AttGRU
class BiF_AGRU(nn.Module):
	def __init__(self, emodict, worddict, embedding, args):
		super(BiF_AGRU, self).__init__()
		self.num_classes = emodict.n_words
		self.embeddings = embedding
		self.gpu = args.gpu
		# sliding window
...
# UniF + Bi-AttGRU
class UniF_BiAGRU(nn.Module):
	def __init__(self, emodict, worddict, embedding, args):
		super(UniF_BiAGRU, self).__init__()
		self.num_classes = emodict.n_words
		self.embeddings = embedding
		self.gpu = args.gpu
		# sliding window
...
# BiF + Uni-AttGRU
class BiF_BiAGRU(nn.Module):
	def __init__(self, emodict, worddict, embedding, args):
		super(BiF_BiAGRU, self).__init__()
		self.num_classes = emodict.n_words
		self.embeddings = embedding
		self.gpu = args.gpu
		# sliding window
	...
	def forward(self, sents, lengths):
		...
		if sents.size()[0] > 1:
			...
			# memory
			mem_out = self.dropout_mid(self.cont_gru(batches_tensor)[0])                    # K x (batch - 1) x 200
			mem_fwd, mem_bwd = mem_out.chunk(2, -1)
			mem_bank = (batches_tensor + mem_fwd + mem_bwd).transpose(0, 1).contiguous()          # (batch - 1) x K x 100
